...
| **KrakenD CE**                | BFF / API gateway declarativo       | buscás validación local de JWT y agregación/BFF con pocas piezas                | no es el caso más natural para una arquitectura basada en auth service externo por request. **Community Edition no soporta WebSocket (es feature Enterprise-only) — si el proyecto necesita proxear WS a través del gateway, KrakenD CE queda descartado para ese tráfico; evaluar Traefik/Kong/Tyk/APISIX, o el patrón de ticket con conexión directa al servicio.** ([KrakenD JWT][1], [KrakenD Docker][10], [API Gateway Pattern][59], [KrakenD Websockets Enterprise][71], [KrakenD CE Websocket Issue][72])                                             |
...
## WebSocket a través del borde

Un WebSocket rompe el supuesto de request/response en el que se apoyan la mayoría de los patrones de auth de esta guía, pero el principio no cambia: el borde autentica y filtra una vez, el dominio decide fino. Lo que cambia es *cuándo* ocurre ese "una vez".

### El handshake es el único punto de auth

Un WebSocket arranca como un `GET` HTTP con headers de `Upgrade`, así que ese upgrade es el único momento donde el borde puede validar el token con las mismas herramientas que ya usa para REST (JWT local, introspection, etc.). Una vez completado, el gateway deja de ver requests individuales — ve frames de un socket ya abierto. Rate limiting y filtrado por request pierden sentido ahí: `Tyk`, por ejemplo, documenta explícitamente que sus límites son por conexión, no por frame, una vez que el socket está arriba. Cualquier control por mensaje (anti-spam, frecuencia por usuario) tiene que vivir en el servicio de dominio, no en el borde — la misma tabla de responsabilidades de más arriba, aplicada a un protocolo distinto. ([Tyk Websockets][74])

### Dos formas de mantener al gateway como único punto de entrada

- **Proxy transparente**: el cliente abre el WS contra el gateway; el gateway valida el token en el upgrade y reenvía los frames sin tocarlos hacia el servicio correspondiente. Mantiene la letra de "único punto de entrada" sin piezas nuevas.
- **Ticket pattern** (el que usan Slack/Discord): el cliente pide, vía un endpoint REST normal ya autenticado por el gateway, un ticket de corta vida; con ese ticket conecta el WS *directo* al servicio de dominio, sin pasar por el gateway. El gateway sigue siendo el único punto de *autenticación*, pero no el único punto de *tráfico* — es una excepción real al patrón por defecto y, como tal, necesita ir documentada en un ADR con el mismo rigor que el resto de las decisiones de esta guía.

### Qué gateways OSS proxean WebSocket

| Producto | ¿Proxea WS? | Nota |
| :--- | :--- | :--- |
| Traefik OSS | Sí, nativo | HTTP, TCP, UDP, gRPC y WebSocket soportados out-of-the-box. ([Traefik Features][5]) |
| Kong OSS | Sí, nativo | Vía upgrade HTTP sobre el Nginx subyacente; los plugins de auth (`JWT`) corren en el handshake, después el proxy es transparente. ([Kong Websocket PDK][73]) |
| Tyk OSS | Sí, desde v2.2 | `enable_websockets: true` en `tyk.conf`; proxy transparente, rate limit por conexión y no por frame. ([Tyk Websockets][74]) |
| Apache APISIX | Sí, nativo | WS es protocolo de primera clase en el routing. ([Apache APISIX][2]) |
| KrakenD CE | **No** | Soporte de WebSocket es feature *Enterprise-only*; la Community Edition no lo tiene. ([KrakenD Websockets Enterprise][71], [KrakenD CE Websocket Issue][72]) |

Si el gateway elegido no proxea WS en su edición gratuita, el ticket pattern deja de ser una alternativa estilística y pasa a ser la única forma de mantener auth centralizada sin pagar una licencia Enterprise.
...
[71]: https://www.krakend.io/docs/enterprise/websockets/
[72]: https://github.com/devopsfaith/krakend-ce/issues/84
[73]: https://developer.konghq.com/gateway/pdk/reference/kong.websocket.client/
[74]: https://tyk.io/docs/3.2/advanced-configuration/websockets/
